...
const SAMPLE_POINTS = [
  { id: 1, name: "New York City, USA", lat: 40.7128, lng: -74.006, priceUSD: 3.89, date: "2025-08-15", source: "USDA (example)", url: "https://example.com/nyc-eggs" },
  { id: 2, name: "San Francisco, USA", lat: 37.7749, lng: -122.4194, priceUSD: 4.59, date: "2025-08-10", source: "USDA (example)", url: "https://example.com/sf-eggs" },
  { id: 3, name: "London, UK", lat: 51.5072, lng: -0.1276, priceUSD: 3.15, date: "2025-08-09", source: "ONS (example)", url: "https://example.com/london-eggs" },
  { id: 4, name: "Melbourne, AU", lat: -37.8136, lng: 144.9631, priceUSD: 4.02, date: "2025-08-18", source: "ABS (example)", url: "https://example.com/melbourne-eggs" },
  { id: 5, name: "Dublin, IE", lat: 53.3498, lng: -6.2603, priceUSD: 3.45, date: "2025-08-05", source: "CSO (example)", url: "https://example.com/dublin-eggs" },
  { id: 6, name: "Tokyo, JP", lat: 35.6762, lng: 139.6503, priceUSD: 4.75, date: "2025-08-12", source: "MAFF (example)", url: "https://example.com/tokyo-eggs" },
  { id: 7, name: "Paris, FR", lat: 48.8566, lng: 2.3522, priceUSD: 3.60, date: "2025-08-14", source: "INSEE (example)", url: "https://example.com/paris-eggs" },
  { id: 8, name: "Berlin, DE", lat: 52.52, lng: 13.405, priceUSD: 3.25, date: "2025-08-17", source: "Destatis (example)", url: "https://example.com/berlin-eggs" },
  { id: 9, name: "Toronto, CA", lat: 43.65107, lng: -79.347015, priceUSD: 4.10, date: "2025-08-11", source: "StatsCan (example)", url: "https://example.com/toronto-eggs" },
  { id: 10, name: "Mexico City, MX", lat: 19.4326, lng: -99.1332, priceUSD: 2.95, date: "2025-08-07", source: "INEGI (example)", url: "https://example.com/mexico-eggs" },
  { id: 11, name: "São Paulo, BR", lat: -23.5505, lng: -46.6333, priceUSD: 2.80, date: "2025-08-16", source: "IBGE (example)", url: "https://example.com/saopaulo-eggs" },
  { id: 12, name: "Cairo, EG", lat: 30.0444, lng: 31.2357, priceUSD: 3.05, date: "2025-08-08", source: "CAPMAS (example)", url: "https://example.com/cairo-eggs" },
  { id: 13, name: "Johannesburg, ZA", lat: -26.2041, lng: 28.0473, priceUSD: 3.70, date: "2025-08-13", source: "StatsSA (example)", url: "https://example.com/johannesburg-eggs" },
  { id: 14, name: "Mumbai, IN", lat: 19.076, lng: 72.8777, priceUSD: 2.65, date: "2025-08-06", source: "MOSPI (example)", url: "https://example.com/mumbai-eggs" },
...
function Button({ children, onClick, className = "", type = "button", disabled }) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`px-3 py-2 rounded-2xl border bg-white hover:bg-gray-50 shadow disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
    >
      {children}
    </button>
  );
...
export default function EggMap() {
  ...
  return (
    <div className="h-screen w-screen relative">
      ...
      {/* Stats card */}
      <div className="absolute top-28 right-3 z-[1000] space-y-3">
        <Card className="border-0">
          <CardContent className="p-3">
            <div className="text-xs text-gray-700">
              ...
              <div className="mt-1">Complaints: {complaints.length}</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-0">
          <CardContent className="p-3">
            <div className="text-sm font-semibold mb-2">Complain about egg prices</div>
            <div className="flex flex-col gap-2">
              <textarea
                value={draftText}
                onChange={(e) => setDraftText(e.target.value)}
                rows={3}
                placeholder="eg. $5.50 a dozen?? highway robbery!"
                className="w-full text-sm p-2 border rounded-lg"
              />
              <div className="text-[11px] text-gray-600">
                Pick a location on the map: <span className="font-mono">{coordLabel(draftLatLng)}</span>
              </div>
              <div className="flex gap-2">
                <Button onClick={() => setPicking((v) => !v)} className={picking ? "ring-2 ring-purple-500" : ""}>
                  📍 {picking ? "Click the map…" : "Pick location"}
                </Button>
                {editingId ? (
                  <>
                    <Button onClick={saveEdit} disabled={!draftText.trim() || !draftLatLng}>💾 Save</Button>
                    <Button onClick={resetDraft}>✖ Cancel</Button>
                  </>
                ) : (
                  <Button onClick={createComplaint} disabled={!draftText.trim() || !draftLatLng}>➕ Add</Button>
                )}
              </div>
            </div>
          </CardContent>
        ...
        {/* Complaints: list (read & quick actions) */}
        {complaints.length > 0 && (
          <Card className="border-0 max-h-56 overflow-auto">
            <CardContent className="p-3">
              <div className="text-sm font-semibold mb-2">Recent complaints</div>
              <ul className="space-y-2">
                {complaints.map((c) => (
                  <li key={c.id} className="text-xs leading-snug">
                    <div className="font-medium line-clamp-2">{c.text}</div>
                    <div className="text-[11px] text-gray-600">
                      {new Date(c.createdAt).toLocaleString()} • {c.lat.toFixed(3)},{c.lng.toFixed(3)}
                    </div>
                    <div className="mt-1 flex gap-2">
                      <Button onClick={() => startEdit(c)} className="px-2 py-1 text-xs">✏️ Edit</Button>
                      <Button onClick={() => deleteComplaint(c.id)} className="px-2 py-1 text-xs">🗑 Delete</Button>
                    </div>
                  </li>
                ))}
              </ul>
      ...
      {isClient && leaflet ? (
        ...
        >
          ...
          {/* Draft pin while picking */}
          {draftLatLng && (
            <leaflet.CircleMarker
              center={[draftLatLng.lat, draftLatLng.lng]}
              radius={8}
              pathOptions={{ color: "#7c3aed", fillOpacity: 0.5, weight: 2, dashArray: "2,4" }}
            >
              <leaflet.Tooltip direction="top" offset={[0, -8]} opacity={1} permanent={false}>
                <div className="text-[11px]">draft</div>
              </leaflet.Tooltip>
              <leaflet.Popup>
                <div className="w-56 text-xs">
                  ...
                  <div className="flex gap-2">
                    {editingId ? (
                      <>
                        <Button onClick={saveEdit} disabled={!draftText.trim()}>💾 Save</Button>
                        <Button onClick={resetDraft}>✖ Cancel</Button>
                      </>
                    ) : (
                      <>
                        <Button onClick={createComplaint} disabled={!draftText.trim()}>➕ Add</Button>
                        <Button onClick={resetDraft}>✖ Cancel</Button>
                      </>
                    )}
                  </div>
          ...
          {complaints.map((c) => (
            <leaflet.CircleMarker
              ...
              center={[c.lat, c.lng]}
              radius={8}
              pathOptions={{ color: "#7c3aed", fillOpacity: 0.7, weight: 2 }}
            >
              <leaflet.Tooltip direction="top" offset={[0, -8]} opacity={1} permanent={false}>
                <div className="text-[11px]">complaint</div>
              </leaflet.Tooltip>
              <leaflet.Popup>
                <div className="w-56 text-xs">
                  <div className="font-semibold mb-1">Complaint</div>
                  <div className="text-gray-800 mb-1 whitespace-pre-wrap">{c.text}</div>
                  <div className="text-[11px] text-gray-500 mb-2">
                    {new Date(c.createdAt).toLocaleString()} • {c.lat.toFixed(4)},{c.lng.toFixed(4)}
                  </div>
                  <div className="flex gap-2">
      ...
      <footer className="fixed bottom-3 left-1/2 -translate-x-1/2 w-[min(980px,92vw)]">
        <Card>
...
export function runEggTests() {
  ...
  assert("localStorage key exists after any mutation (cannot auto-check here)", true);
...
if (typeof window !== "undefined") {
  try {
    const shouldRun =
      window.__EGG_RUN_TESTS__ === true ||
      (typeof window.location?.hash === "string" && window.location.hash.includes("run-tests"));
    if (shouldRun) {
